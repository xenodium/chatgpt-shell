...
There are currently two shell implementations (ChatGPT and DALL-E). Other services (local or cloud) can be brought to Emacs as shells. =shell-maker= should help with that. Both =chatgpt-shell= and =dall-e-shell= use =shell-maker=, but a basic shell implementation looks as follows:
