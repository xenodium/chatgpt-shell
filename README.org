...
chatgpt-shell is a [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Shell-Prompts.html][comint]] shell, so your favourite Emacs shell flows also apply to this buffer.
...
Below is a handy integration to extract Japanese vocabulary. There's also a generic image descriptor available via =M-x chatgpt-shell-describe-image= that works on Emacs buffer (dired, image buffer, point on image, or selecting a desktop region).
