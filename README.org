...
=chatgpt-shell= depends on =shell-maker=. This dependency is tesolved without issues on MELPA but seems to run into issues with =straight=. I'm not familiar with =straight= but users have reported the following to work. If you have a better solution, please send a pull request or open an issue with the suggestion.
